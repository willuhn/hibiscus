...
public class VerwendungszweckUtil
{
   ...
   * @param lines
   *          die Zeilen.
   * @return Ein Array mit den Zeilen. Niemals NULL sondern hoechstens ein leeres Array.
   */
  ...
  public static void applyText(HibiscusTransfer t, String text) throws RemoteException
  {
    ...
    String zweck = tokenizer.getNextText(27);
    if (zweck.isEmpty())
    {
      return;
    }
    ...
    String zweck2 = tokenizer.getNextText(27);
    if (zweck2.isEmpty())
    {
      return;
    }
    ...
    int length = 0;
    List<String> weitereVerwendungszwecke = new ArrayList<>();

    while (true)
    {

      String zweckn = tokenizer.getNextText(35);
      if (zweckn.isEmpty())
      {
        break;
      }
      weitereVerwendungszwecke.add(zweckn);
      length += zweckn.length();
    }
    if (!weitereVerwendungszwecke.isEmpty())
    {
      t.setWeitereVerwendungszwecke(weitereVerwendungszwecke.toArray(new String[0]));
    }
  }
  ...
  static class Tokenizer
  {
    ...
    private final LinkedList<String> m_Words;

    private int m_Pos;

    Tokenizer(String text)
    {
      m_Words = new LinkedList<>(Arrays.stream(text.split("\\s+")).filter(w -> !StringUtils.isBlank(w)).collect(Collectors.toList()));
    }

    public String getNextText(int length)
    {
      ...
      String firstWord = m_Words.removeFirst();
      if (firstWord.length() > length)
      {

        String ret = StringUtils.substring(firstWord, 0, length);
        m_Words.addFirst(StringUtils.substring(firstWord, length));
        return ret;
      }

      StringBuilder ret = new StringBuilder();
      ret.append(firstWord);

      while (!m_Words.isEmpty())
      {

        String newWord = m_Words.peek();

        if (ret.length() + 1 + newWord.length() > length)
        {
          break;
        }
   ...
   * 
   * @param t
   *          der Auftrag, in dem die Verwendungszweck-Zeilen gespeichert werden sollen.
   * @param lines
   *          die zu uebernehmenden Zeilen.
   * @throws RemoteException
   */
  public static void apply(HibiscusTransfer t, String[] lines) throws RemoteException
  {
    ...
    t.setZweck(StringUtils.substring(all, 0, 27));
    if (all.length() > 27)
    {
      t.setZweck2(StringUtils.substring(all, 27, 54)); //
      if (all.length() > 54)
      {
        t.setWeitereVerwendungszwecke(new String[] { StringUtils.substring(all, 54, 54 + 35) });
      }
    }
  }
  ...
  /**
   * Bricht die Verwendungszweck-Zeilen auf $limit Zeichen lange Haeppchen neu um. Jedoch nur, wenn wirklich Zeilen enthalten sind, die laenger sind.
   * Andernfalls wird nichts umgebrochen.
   * 
   * @param limit
   *          das Zeichen-Limit pro Zeile.
   * @param lines
   *          die Zeilen.
   ...
   * @param lines
   *          die Zeilen.
   * @return die gemergten Zeilen. Wird NULL oder ein leeres Array uebergeben, liefert die Funktion NULL.
   */
   ...
   *          der Auftrag.
   * @param sep
   *          das zu verwendende Trennzeichen fuer die Zeilen. Wenn es null ist, wird " " (ein Leerzeichen) verwendet.
   * @return der String mit einer Zeile, die alle Verwendungszwecke enthaelt.
   * @throws RemoteException
   */
  ...
  /**
   * Bereinigt die Verwendungszweck-Zeilen. Hierbei werden leere Zeilen oder NULL-Elemente entfernt. Ausserdem werden alle Zeilen getrimt.
   * 
   * @param trim
   *          wenn die Zeilen-Enden getrimmt werden sollen.
   * @param lines
   *          die zu bereinigenden Zeilen.
