    ...
    try
    {
      if (object instanceof AuslandsUeberweisung)
      {
        ...
        if (ueb.isTerminUeberweisung())
          return i18n.tr("{0}: ({1}) {2} {3} per {4} an {5} ({6}) überweisen",k.getLongName(),VerwendungszweckUtil.evaluate(ueb.getZweck()),HBCI.DECIMALFORMAT.format(ueb.getBetrag()),k.getWaehrung(),HBCI.DATEFORMAT.format(ueb.getTermin()),ueb.getGegenkontoName(),HBCIProperties.formatIban(ueb.getGegenkontoNummer()));

        if (ueb.isUmbuchung())
          return i18n.tr("{0}: ({1}) {2} {3} an {4} ({5}) überweisen (Umbuchung)",k.getLongName(),VerwendungszweckUtil.evaluate(ueb.getZweck()),HBCI.DECIMALFORMAT.format(ueb.getBetrag()),k.getWaehrung(),ueb.getGegenkontoName(),HBCIProperties.formatIban(ueb.getGegenkontoNummer()));
        if (ueb.isInstantPayment())
          return i18n.tr("{0}: ({1}) {2} {3} an {4} ({5}) überweisen (Echtzeitüberweisung)",k.getLongName(),VerwendungszweckUtil.evaluate(ueb.getZweck()),HBCI.DECIMALFORMAT.format(ueb.getBetrag()),k.getWaehrung(),ueb.getGegenkontoName(),HBCIProperties.formatIban(ueb.getGegenkontoNummer()));

        return i18n.tr("{0}: ({1}) {2} {3} an {4} ({5}) überweisen",k.getLongName(),VerwendungszweckUtil.evaluate(ueb.getZweck()),HBCI.DECIMALFORMAT.format(ueb.getBetrag()),k.getWaehrung(),ueb.getGegenkontoName(),HBCIProperties.formatIban(ueb.getGegenkontoNummer()));
      }
      ...
      if (object instanceof SepaDauerauftrag)
      {
        ...
        return i18n.tr("{0}: ({1}) {2} {3} an {4} ({5}), Turnus: {6}",k.getLongName(),VerwendungszweckUtil.evaluate(dauer.getZweck()),HBCI.DECIMALFORMAT.format(dauer.getBetrag()),k.getWaehrung(),dauer.getGegenkontoName(),HBCIProperties.formatIban(dauer.getGegenkontoNummer()),dauer.getTurnus().getBezeichnung());
      }
      ...
      if (object instanceof SepaLastschrift)
      {
        ...
        return i18n.tr("{0}: ({1}) {2} {3} von {4} ({5}) einziehen",k.getLongName(),VerwendungszweckUtil.evaluate(last.getZweck()),HBCI.DECIMALFORMAT.format(last.getBetrag()),k.getWaehrung(),last.getGegenkontoName(),HBCIProperties.formatIban(last.getGegenkontoNummer()));
      }
